...
import 'package:legends/test1.dart';

void xmppManager() async {
  print("Type user@domain:");
//  var userAtDomain = stdin.readLineSync(encoding: utf8);
  var userAtDomain = 'bot@legends.com';
  print("Type password");
  var password = 'qwerty';
//  print("Type port");
//  int port;
...
class ExampleConnectionStateChangedListener
  ...
  void onConnectionStateChanged(xmpp.XmppConnectionState state) {
    if (state == xmpp.XmppConnectionState.DoneServiceDiscovery) {
      ...
      var receiver = "k4droid3@legends.com";
      ...
      messageByteStream.map((bytes) {
        var str = ascii.decode(bytes);
        str = str.substring(0, str.length - 1);
        return str;
      }).listen((String str) async {
        await messageHandler.sendMessage(receiverJid, str);
      });
    }
  }
}
